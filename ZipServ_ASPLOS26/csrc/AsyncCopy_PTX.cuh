...
template<int SizeInBytes>
__device__ __forceinline__ void cp_async(uint8_t* smem_ptr, const uint8_t* global_ptr, bool pred_guard = true)
{
    static_assert((SizeInBytes == 4 || SizeInBytes == 8 || SizeInBytes == 16), "Size is not supported");
    unsigned smem_int_ptr = __cvta_generic_to_shared(smem_ptr);
    asm volatile("{ \n"
...
// Template specialization for cp_async with __nv_bfloat16
template<int SizeInBytes>
